...
pub trait Cache<V> where V:ValueInterface{
    /// Clean the cache
...
impl<V> Cache<V> for NoCache<V> where V:ValueInterface{
    fn clear(&mut self) {}
...
#[derive(Debug, Clone)]
pub struct MemoryCache<V:ValueInterface>(
    HashMap<String,State<V>>
);
...
impl<V> Cache<V> for MemoryCache<V> where V:ValueInterface{
    fn clear(&mut self) {
        self.0.clear();
    }
    ...
    fn set_metadata(&mut self, metadata:&Metadata)->Result<(), Error> {
        ...
        if let Some(state) = self.get(&key){
            ...
            self.0.insert(key, State::new().with_metadata(metadata.clone()));
        }
        Ok(())
    }
