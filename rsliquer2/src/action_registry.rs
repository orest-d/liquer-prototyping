    ...
    <T as std::convert::TryInto<i32>>::Error: Display,
{
    fn call_action(&self, input: T, _arguments: &Vec<ActionParameter>) -> Result<T, Error> {
        let f_input: i32 = input.try_into().map_err(|e| Error::ConversionError {
            message: format!("Input argument conversion failed; {}", e),
        })?;

        let out: i32 = (*self)(f_input);
        let result: T = out.try_into().map_err(|e| Error::ConversionError {
...
{
    fn call_action(&self, input: T, _arguments: &Vec<ActionParameter>) -> Result<T, Error> {
        let f_input: In = input.try_into().map_err(|e| Error::ConversionError {
            message: format!("Input argument conversion failed; {}", e),
        })?;
...
pub struct HashMapActionRegistry<T>(HashMap<String, HashMap<String, Box<dyn CallableAction<T>>>>);
...
impl<T> HashMapActionRegistry<T> {
    ...
    pub fn register_callable_action(
        &mut self,
        ns: &str,
        name: &str,
        action: Box<dyn CallableAction<T>>,
    ) {
        let ns = ns.to_owned();
    ...
    pub fn call(
        ...
        self.0
            .get(ns)
            .ok_or_else(|| Error::ActionNotRegistered {
                message: format!(
                    "Action {} not registered in namespace {}; no such namespace",
                    name, ns
                ),
            })
            .and_then(|ns_registry| {
                ns_registry
                    .get(name)
                    .ok_or_else(|| Error::ActionNotRegistered {
