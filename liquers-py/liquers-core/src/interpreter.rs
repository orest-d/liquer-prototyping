        ...
        if let Some(plan) = &mut self.plan {
            if let Some(step) = plan.steps.get(self.step_number) {
                match step{
                    ...
                    crate::plan::Step::Info(m) => {self.metadata.as_mut().unwrap().info(&m);},
                    crate::plan::Step::Warning(m) => {self.metadata.as_mut().unwrap().warning(&m);},
                    crate::plan::Step::Error(m) => {self.metadata.as_mut().unwrap().error(&m);},
